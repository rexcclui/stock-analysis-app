...
export function clearAllClientCache() {
  try {
    ...
    console.log(`[Client Cache] ✓ Cleared all ${cleared} cache entries`);
    return cleared;
  } catch (error) {
    console.error('[Client Cache] Error clearing all cache:', error);
    return 0;
  }
}
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        throw new Error('Rate limit exceeded. Please wait a moment and try again, or use cached data.');
